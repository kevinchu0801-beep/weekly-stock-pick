...
- `npx -y westock-data-clawhub@1.0.4 --help` 可运行
...
westock-data 调用方式：
```bash
WD="npx -y westock-data-clawhub@1.0.4"
```
...
每只入选股票必须记录因子分解和质量标签：
- `factorMomentum` / `factorFund` / `factorCatalyst` / `factorValuation`
- `dataQuality`: 高 / 中 / 低
- `conviction`: A / B / C
- `riskPenalty`: 拥挤、波动、趋势破位等风险降权合计；风险降权不作为硬剔除，只影响排序、仓位和卡片解释
...
- **westock-data 旧路径失效**：优先使用 `npx -y westock-data-clawhub@1.0.4`；不要再把旧本地脚本路径当成强依赖
...
7. ☑ HTML 筛选、排序、折叠按钮可用；卡片包含四因子分解、数据完整度、判断置信度、风险降权
